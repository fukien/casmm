...
template <typename IT, typename NT>
toy_time_t OuterSpGEMM_stage(const CSC<IT, NT>& A, const CSR<IT, NT>& B, IT startIdx, IT endIdx, CSR<IT, NT>& C, \
    int nblockers, int nblockchars, int partition)
{
    typedef tuple<IT, IT, NT> TripleNode;
    const uint16_t nthreads = omp_get_max_threads();

    double symb_start_time = omp_get_wtime();
	...
	do_static_symbolic(A, B, IT(0), A.cols, nrows_per_blocker, num_blockers, flops_by_row_blockers, total_flop);

    double symb_end_time = omp_get_wtime();

    double numc_start_time = omp_get_wtime();
...
#pragma omp parallel for
    ...
    double numc_end_time = omp_get_wtime();

    toy_time_t toy_time;
    toy_time.symb_time = symb_end_time - symb_start_time;
    toy_time.numc_time = numc_end_time - numc_start_time;
    return toy_time;
}

template <typename IT, typename NT>
toy_time_t OuterSpGEMM(const CSC<IT, NT>& A, const CSR<IT, NT>& B, CSR<IT, NT>& C, int nblockers, int nblockchars, int partition)
{
...
//     }
    return OuterSpGEMM_stage(A, B, IT(0), A.cols, C, nblockers, nblockchars, partition);
}
